...
5. If `knowz` is on PATH, `knowz vault list --json`. Else if Knowz MCP is available, `mcp__knowz__list_vaults` with `includeStats: true`. If neither, report that Knowz enhancement is unavailable and the local workflow still works. Grok Build connect: `grok plugin install knowz-io/cursor-knowz-plugin#plugins/knowz --trust`, then `knowz login` or `/mcps` → **knowz** → `i`. Do not paste API keys in Grok Bot chat. Never suggest a bare `grok plugin install knowz`.
6. Evaluate relay status using the fixed `RELAY_HOST=grok` rules below.
7. End with one practical action: initialize, continue work, authenticate/install a target CLI, flush captures, or (only if they want vaults) Grok Build `grok plugin install knowz-io/cursor-knowz-plugin#plugins/knowz --trust` / Grok Bot or Cursor → search **Knowz** → **Add** → **Authorize**.
...
MCP & vaults: {CLI | MCP connected — N vault(s) | Not connected (optional — workflow still works)}
Relay host: grok
