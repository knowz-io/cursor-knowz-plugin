...
**Grok Build** (this Grok TUI / agent session): skills come from this plugin after `grok plugin install knowz-io/cursor-knowz-plugin#plugins/knowzcode --trust`. Bootstrap framework files with `npx --yes knowzcode install --target "{absolute-repository-root}" --force` — do not require `.cursor/rules`. Copy this plugin's `rules/knowzcode.md` to `{repo}/.grok/rules/knowzcode.md` when that file is missing. See `docs/grok-build.md` in the marketplace repo. `command -v grok` does **not** prove this session is Grok Build.
...
2. Bootstrap through the published CLI rather than assuming this skill can find a plugin-relative framework directory.
   Do **not** infer the host from `command -v grok` (Cursor users often have Grok Build on PATH). Detect surfaces independently:
   - Grok session: this is Grok Build/TUI (GROK_HOME / plugin session), or the repo already has `.grok/`.
   - Cursor session: Cursor IDE / Grok Bot marketplace agent, or the repo already has `.cursor/`.
   - If both apply, do both. If neither is obvious, ask.
   - Fresh repository: `npx --yes knowzcode install --target "{absolute-repository-root}" --force`
   - Also `--platforms cursor` when a Cursor surface applies (fresh install or `knowzcode add-platforms --platforms cursor --force` on an existing tree).
...
3. Preserve user-authored project files. The CLI's ownership marker and manifest control which Cursor surfaces it may update (including `.cursor/rules/knowzcode.mdc`). Do not fail setup if `.cursor/rules` is absent on a Grok-only tree.
...
6. Host-specific always-on rule (independent checks — a machine can need both):
   - **Grok:** if `{repo}/.grok/rules/knowzcode.md` is missing, copy this plugin's `../../rules/knowzcode.md` there (create `.grok/rules/` as needed). Grok loads `*.md` from `.grok/rules/` automatically.
   - **Cursor:** confirm `.cursor/rules/knowzcode.mdc` exists after a Cursor-platform install. This plugin also ships `rules/knowzcode.mdc` so Cursor / Grok Bot agents get the methodology without a local copy.
7. If the user also wants team memory, mention the **Knowz** plugin as a **separate** install (do not merge it into this workflow). Grok Build: `grok plugin install knowz-io/cursor-knowz-plugin#plugins/knowz --trust`, then CLI (`knowz login`) or `/mcps` → **knowz** → `i`. Grok Bot / Cursor: search **Knowz** → **Add** → **Authorize**. Do not run `/knowz register` on an existing account. Do not paste API keys in Grok Bot chat. Never suggest a bare `grok plugin install knowz`.
